// Wind layer helpers: load the wind grid (live GFS or ERA5 average) and
// render it to an equirectangular canvas (speed colour + direction arrows)
// used as the globe surface texture.

// "live" -> public/wind.json, refreshed from NOAA GFS every 6h
//   (scripts/fetch_gfs_wind.py + .github/workflows/refresh-wind.yml).
// "average" -> public/wind-avg.json, a static 1995-2025 ERA5 climatology
//   (scripts/fetch_era5_wind.py, run by hand — it doesn't change often).
const WIND_FILES = { live: "/wind.json", average: "/wind-avg.json" };

export async function loadWind(variant = "live") {
  const file = WIND_FILES[variant] || WIND_FILES.live;
